Class ("paella.RTMPVideo", paella.VideoElementBase,{
	...
	_duration:0,
	_paused:true,
	//#DCE include a stream name to get qualities
	_streamName:null,
	_flashId:null,
	_swfContainer:null,
	_flashVideo:null,
	...
	initialize:function(id,stream,left,top,width,height) {
		...
		//#DCE get the stream name to support getQualities
	...
	setQuality:function(index) {
		...
		return new Promise((resolve,reject) => {
			...
			// #DCE enable changing source (stream resolution) on Live events, reset states
			this._ready = false;
			this._isReady = false;
			this.load()
				.then(function() {
